...
// 保持するのはシグネチャの構築に要る相手クラスのみで、生成先自身は生成時のコールバックが持つ
sealed interface EnumizeGenerationRole {
    ...
    object GeneratedEnumishCompanion : EnumizeGenerationRole

    // kind となる companion（既存・生成とも）: label / enumizedClass を宣言する
    class KindCompanion(val leaf: FirRegularClassSymbol) : EnumizeGenerationRole

    // 末端 object / data object（kind 自身）: label / enumizedClass / asEnumish を宣言する
    object LeafObject : EnumizeGenerationRole
